...
class TemplateMixin(object):
    DEFAULT_TITLE = '自动化测试报告默认标题'

    # ------------------------------------------------------------------------
    # HTML Template

    HTML_TEMP1 = r"""<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
                        ...
                        <div class="col-sm-6 b-r" style="height:350px">
                            <form class="form-horizontal">
                                ...
                                <div class="form-group">
                                    <label class="col-sm-2 control-label text-navy">用例标题:</label>
                                    <div class="col-sm-5">
                                        <span class="form-control" id="testName">测试</span>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label class="col-sm-2 control-label text-navy">用例总数:</label>
                                    <div class="col-sm-5">
                                        <span class="form-control" id="testAll">1</span>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label class="col-sm-2 control-label text-navy">用例通过:</label>
                                    <div class="col-sm-5">
                                        <span class="form-control" id="testPass">1</span>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label class="col-sm-2 control-label text-danger">用例失败:</label>
                                    <div class="col-sm-5">
                                        <span class="form-control text-danger" id="testFail">0</span>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label class="col-sm-2 control-label text-warning">用例跳过:</label>
                                    <div class="col-sm-5">
                                        <span class="form-control text-warning" id="testSkip">0</span>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label class="col-sm-2 control-label text-navy">开始时间:</label>
                                    <div class="col-sm-5">
                                        <span class="form-control" id="beginTime">2019-03-01 10:43:58</span>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label class="col-sm-2 control-label text-navy">运行时间:</label>
                                    <div class="col-sm-5">
                                        <span class="form-control" id="totalTime">1174ms</span>
                                    </div>
                                </div>
                            </form>
                        </div>
                        <div class="col-sm-6">
                            <div style="height: 350px; -webkit-tap-highlight-color: transparent; user-select: none; position: relative; background: transparent;" id="echarts-map-chart" _echarts_instance_="ec_1597225521216">
                                <div style="position: relative; overflow: hidden; width: 905px; height: 350px; padding: 0px; margin: 0px; border-width: 0px; cursor: default;"><canvas width="905" height="350" data-zr-dom-id="zr_0" style="position: absolute; left: 0px; top: 0px; width: 905px; height: 350px; user-select: none; -webkit-tap-highlight-color: rgba(0, 0, 0, 0); padding: 0px; margin: 0px; border-width: 0px;"></canvas>
                                </div>
                                <div style="position: absolute; display: none; border-style: solid; white-space: nowrap; z-index: 9999999; transition: left 0.4s cubic-bezier(0.23, 1, 0.32, 1) 0s, top 0.4s cubic-bezier(0.23, 1, 0.32, 1) 0s; background-color: rgba(50, 50, 50, 0.7); border-width: 0px; border-color: rgb(51, 51, 51); border-radius: 4px; color: rgb(255, 255, 255); font: 14px / 21px &quot;Microsoft YaHei&quot;; padding: 5px; left: 479px; top: 245px;">
                                    运行结果
                                    <br>成功 : 1 (100%)
                                </div>
                            </div>
            ...
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    <div class="input-group panel-heading" style="width: 100%; background-color: #1ab394; margin-bottom: 10px; text-align: left; font-family: Consolas;">
                        <label style="color: snow">用例名称:</label>
                    ...
                    <table class="table table-bordered" style="word-wrap:break-word; word-break:break-all;">
                        ...
                        <tr>
                            ...
                            <th width="39%" align="center">测试过程</th>
                            <th width="3%" align="center">结果</th>
...
<script type="text/javascript">
    ...
    function details(obj) {
        ...
        if ($(obj).text() == '展开') {
            ...
            $.each(logs, function (i, n) {
                if(i != 'type'){
                    // 不展示type字段
                    detailLog = detailLog + "<tr class='show_log' >+<td></td>+<td style='white-space:nowrap;background-color: #ecfcff'>"+'详细步骤'+"</td><td colspan='6' style='background-color: #ecfcff'>"+n+"</td></tr>";
                }
            });
            $(obj).parent().parent().after(detailLog)
        } else if ($(obj).text() == '收缩') {
            var count = 1;
            if(type == 'error'){
                // 当type为error时，表示代码出错，此条详情仅有content一个字段，只向后删除一个节点
                count = 1;
            }
    ...
    $(function () {
        ...
        $.each(results, function (i, n) {
            ...
            $("#filterResult").append("<option value='" + n + "' hassubinfo='true'>" + m + "</option>");
        });
        ...
        function generateResult(className, caseResult) {
            ...
            $.each(resultData["testResult"], function (i, n) {
                if ((className == "" || n["className"] == className) && (caseResult == "" || n["status"] == caseResult)) {
                    ...
                    } else if (n["status"] == 'error' || n["status"] == 'fail' || n['status'] == undefined) {
                        filterFail += 1;
                    ...
                    //处理内容过长的字段，最多显示300个字符
                    var response_body = n['log']["response_body"];
                    if(response_body.length > 230){
                            response_body = response_body.substring(0,230) + " ......"
                    }
                    var request_body = n['log']["request_body"];
                    var tr = "<tr style='font-family: Consolas'>" +
                        "<td>" + (i + 1) + "</td>" +
                        "<td>" + n["className"] + "</td>" +
                        "<td>" + response_body + "</td>" +
                        status +"<td><button data-type='" + n['log']['type'] + "' type='button' onclick='details(this)' buttonIndex='" + i + "' class='btn btn-primary btn-xs' style='margin-bottom: 0px'>展开</button></td></tr>"
                        // 将此条用例的是否代码出错进行标记，type字段
...
class _TestResult(TestResult):
    ...
    def addSuccess(self, test):
        ...
        output = self.complete_output().replace('\n', '</br>')
        self.result.append({
            "className": str(test),
            "status": "success",
            "log": {"response_body": output},
        })
        if self.verbosity > 1:
            sys.stderr.write('ok ')
    ...
    def addError(self, test, err):
        ...
        self.result.append({
            "className": str(test),
            "status": "skip",
            "log": {"response_body": output + _exc_str.replace('\n', '</br>')},
        })
        if self.verbosity > 1:
    ...
    def addFailure(self, test, err):
        ...
        self.result.append({
            "className": str(test),
            "status": "fail",
            "log": {"response_body": output + '</br>' + _exc_str.replace('\n', '</br>')},
        })
        if self.verbosity > 1:
...
class HTMLTestRunner(TemplateMixin):
    ...
    def generate_report(self, result):
        ...
        temp1_data = {"testPass": result.success_count,
                      "testAll": total,
                      "testFail": result.failure_count,
                      "testSkip": result.error_count,
                      "beginTime": start_time,
                      "totalTime": duration,
                      "testName": self.title,
                      "testResult": result.result
                      }
        html_data = self.HTML_TEMP2 % dict(resultData=temp1_data)
        result_data = self.HTML_TEMP1 + html_data + self.HTML_TEMP3
        self.stream.write(result_data.encode('utf8'))
